...
// modified the recursion to while loop.
// but it resulted in over time.
...
void run(int y, int x, int d, bool first){

    while(1) {
        if (first == false && y == start_y && x == start_x) return;
        if (y>=0 && y<N && x>=0 && x<N && map[y][x] == -1) return; //black hole
